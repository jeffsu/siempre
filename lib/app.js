(function() {
  var Controller, TAIL_TIMEOUT, app, config, configFile, controller, errorHandler, express, http, io, methodOverride, morgan, parser, path, server;
  ...
  app.post('/processes/:name/stop', function(req, res, next) {
    var name;
    name = req.params.name;
    return controller.stop(name, function(err) {
      if (err) {
        return next(err);
      }
      return res.json({});
  ...
  app.post('/processes/:name/restart', function(req, res, next) {
    var name;
    name = req.params.name;
    return controller.restart(name, function(err) {
      if (err) {
        return next(err);
      }
      return res.json({});
